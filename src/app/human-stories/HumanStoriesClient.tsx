...
const COVER = "/releases/hs-album.png";
const ACCENT = "#C7B896";
...
function TrackCard({ track }: { track: Track }) {
  ...
  return (
    <div className="card-glow w-full" style={{ color: track.accent }}>
      ...
      >
        <Link href={`/human-stories/${track.slug}`} className="aspect-square w-full relative overflow-hidden block">
    ...
    <>
      ...
      <main className="pt-[60px] min-h-screen font-sans" style={{ background: "#0d1117" }}>
        ...
        <div id="tracks" style={{ background: "#0d1117" }}>
          ...
          {viewMode === "coverflow" ? (
            <CoverFlow album={humanStoriesAlbum} initialSlug={initialSlug} />
          ) : (
            <section className="pt-0 pb-12 px-6 max-w-[1400px] mx-auto">
              <ScrollReveal stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {humanStoriesAlbum.tracks.map((track) => (
        ...
        <section className="relative w-full overflow-hidden">
          <img
            src="/releases/hs-tracklist.png"
            alt="Human Stories — tracklist"
