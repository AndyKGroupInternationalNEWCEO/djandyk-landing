...
function TrackCard({ track }: { track: Track }) {
  ...
  return (
    <div className="card-glow w-full" style={{ color: track.accent }}>
      ...
      >
        <Link href={`/before-i-forget/${track.slug}`} className="aspect-square w-full relative overflow-hidden block">
          {track.videoUrl ? (
            <video
              src={track.videoUrl}
        ...
        <div className="p-4">
          ...
          <Link
            href={`/before-i-forget/${track.slug}`}
    ...
    <>
      ...
      <main className="pt-[60px] min-h-screen font-sans" style={{ background: "#0d1117" }}>
        ...
        <div id="tracks" style={{ background: "#0d1117" }}>
          ...
          {viewMode === "coverflow" ? (
            <CoverFlow album={beforeIForgetAlbum} initialSlug={initialSlug} />
          ) : (
            <section className="pt-0 pb-12 px-6 max-w-[1400px] mx-auto">
              <ScrollReveal stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {beforeIForgetAlbum.tracks.map((track) => (
